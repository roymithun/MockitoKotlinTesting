...
class CalculatorUnitTest {
    ...
    fun `Dividing by zero should throw the DivideByZeroException`() {
        val exception: DivideByZeroException = Assertions.assertThrows(DivideByZeroException::class.java) {
    ...
    fun `The square of a number should be equal to that number multiplied in itself`() {
        Assertions.assertAll(
            ...
            Executable { Assertions.assertEquals(81, calculator.square(9)) }
        )
    }
    ...
    fun testSquaresFactory() = listOf(
        ...
        DynamicTest.dynamicTest("when I calculate 9^2 then I get 81") {
            Assertions.assertEquals(
                81,
                calculator.square(9)
            )
    ...
    @TestFactory
    fun testSquareRootFactory3() = squaresTestData
        .map { (expected, input) ->
    ...
    // Read More: https://javabydeveloper.com/junit-5-tag-and-filtering-tags-with-examples/
    @Tags(
        Tag("slow"),
        Tag("logarithms")
    )
